...
export default function CommentsModal({ _id }: Type) {
  ...
  const handleAddComment = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const res = await api.post(`/api/story/add-comment/${_id}`, { text: formData })

      await api.get(`/api/story/get-comments/${_id}`)
  ...
  const getComments = async () => {
    ...
    try {
      ...
      const res = await api.get(`/api/story/get-comments/${_id}`)
  ...
  return (
    <dialog id={`comment-${_id}`} className="modal backdrop-blur-md cursor-auto">
