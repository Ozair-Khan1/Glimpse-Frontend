            ...
            <div className="modal-box w-11/12 max-w-2xl backdrop-blur-md h-9/12 p-5 bg-[#22262B] flex flex-col items-center align-middle mt-2 gap-15 overflow-hidden">
                ...
                {step === 1 ? (
                    <div className="flex flex-col justify-center items-center align-middle h-full w-full overflow-hidden gap-5">
                        <div className='avatar size-50 w-fit h-fit flex flex-col justify-center items-center gap-3'>
                        ...
                        <div className="flex flex-col gap-2 justify-center items-center align-middle">
                            ...
                            {fileError && <p className="text-red-500 font-semibold">File size should be less than 100MB</p>}
                        </div>
                    </div>
                ) : (
                    <div className="flex flex-col gap-5 h-full w-full align-middle items-center overflow-hidden rounded-lg">
                        {previews.map((file, index) => (
                            <div key={index} className="relative flex-1 flex flex-col gap-5 items-center hide-preview h-full w-full justify-center bg-black">
                                {file.type === 'video' ? (
                        ...
                        <button className="btn btn-success text-white" onClick={handlePost} disabled={loading}>{loading ? <span className="loading loading-infinity loading-xl"></span> : 'Add story'}</button>
                    </div>
                )}
            </div>
        </dialog>
