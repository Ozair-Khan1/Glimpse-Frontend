    ...
    <div className="flex flex-col items-center w-full min-h-screen text-white overflow-x-hidden lg:mt-0 md:mt-0 sm:mt-0 mt-15">
      ...
      <div className="max-w-[1450px] reduce-profile-width w-full h-full flex justify-center items-center align-middle flex-wrap gap-3 pb-16">
        {loading ? (
          ...
          post.length > 0 ? (
            post.map((User) => (
              <div key={User._id} className="relative aspect-9/12 group max-w-[250px] lg:max-w-[300px] md:max-w-[300px] w-full lg:max-h-[450px] md:max-h-[450] max-h-[350px] h-full cursor-pointer bg-neutral-900" onClick={() => {
                (document.getElementById(`${User._id}`) as HTMLDialogElement)?.showModal()
                const video = document.getElementById(`${User._id}`)?.querySelector('video');
                if (video) video.play();
              }}>
                {User.imageUrl?.split('?')[0].match(/\.(mp4|webm|ogg|mov)$/i) ? (
                  <video
                    src={`${User.imageUrl}#t=0.001`}
